...
def run_pipeline(user_function,
    ...
    PIPELINE1 = 'v4l2src device=/dev/video1 ! {src_caps} ! {leaky_q} '
    ...
    SINK_ELEMENT = 'appsink name=appsink sync=false emit-signals=true max-buffers=10 drop=true'
    SINK_ELEMENT1 = 'appsink name=appsink1 sync=false emit-signals=true max-buffers=10 drop=true'
 
    DL_CAPS = 'video/x-raw,format=RGBA,width={width},height={height}'
    SINK_CAPS = 'video/x-raw,format=RGB,width={width},height={height}'
    LEAKY_Q = 'queue max-size-buffers=200 leaky=downstream'

    src_caps = SRC_CAPS.format(width=src_size[0], height=src_size[1])
    src_caps1 = SRC_CAPS1.format(width=src_size[0], height=src_size[1])
    ...
    pipeline += " "+  PIPELINE1.format(leaky_q=LEAKY_Q,
                src_caps=src_caps1, dl_caps=dl_caps, sink_caps=sink_caps,
                sink_element=SINK_ELEMENT1)
    ...
    overlay1 = pipeline.get_by_name('overlay1')
    appsink1 = pipeline.get_by_name('appsink1')
    appsink1.connect('new-sample', partial(on_new_sample,
        overlay=overlay1, screen_size = src_size,
        appsink_size=appsink_size, user_function=user_function))
