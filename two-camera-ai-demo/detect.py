...
def main():
    ...
    drive, path_and_file = os.path.splitdrive(args.model)
    path, files = os.path.split(path_and_file)

    engine = DetectionEngine(args.model)
    labels = load_labels(args.labels)

    if files == "mobilenet_ssd_v2_coco_quant_postprocess_edgetpu.tflite":
        model_name = "mobilenet_ssd v2"
        detection = "Object Detection"
    elif files == "mobilenet_ssd_v2_face_quant_postprocess_edgetpu.tflite":
        model_name = "mobilenet_ssd v2"
    ...
    def user_callback(image, svg_canvas):
      ...
      text_lines = [
          '',
          'Tinker Edge T' ,
          'ML model: %s' %(model_name),
          '%s' %(detection),
          'Inference time/FPS: %.2f ms / %.2f' %((end_time - start_time) * 1000,1.0/(end_time - start_time)),
